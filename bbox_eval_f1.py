...
def get_args():
    ...
    parser.add_argument("--gt-json", type=str, required=True, help="Path to ground truth json file.")
    parser.add_argument("--pred-json", type=str, required=True, help="Path to predicted json file.")
    parser.add_argument("--output-dir", type=str, required=True, help="Path to output directory.")
    parser.add_argument("--tp-distance", type=int, default=20, help="Distance pixel threshold for true positives.")
    ...
    for img_id, gt_anns in gt_to_img_map.items():
        ...
        if len(frame_anns) > 0:
            for i, (gt_matched, gt_ann) in enumerate(zip(gt_matches, gt_anns)):
                ...
                for j, (pred_matched, pred_ann) in enumerate(zip(pred_matches, frame_anns)):
                    pred_mid_x = pred_ann['bbox'][0] + pred_ann['bbox'][2] / 2
                    pred_mid_y = pred_ann['bbox'][1] + pred_ann['bbox'][3] / 2

                    dist = ((pred_mid_x - gt_mid_x)**2 + (pred_mid_y - gt_mid_y)**2)**0.5
...
def plot_f1_curve(confs, scores, output_dir, plot_name="f1_curve.png"):
    ...
    plt.plot(confs, scores, label=f"Max F1: {scores[max_score]:.4f} at {confs[max_score]:.2f}")
...
def main(args):
    ...
    print(f"gt_json: {len(gt_json['annotations'])}, pred_json: {len(conf_detections[max_conv])}")
    print(f"tp: {stats['tp']}, fp: {stats['fp']}, fn: {stats['fn']}")
    print(f"f1: {max_score} at confidence {max_conv:.2f}")

    plot_f1_curve(list(conf_f1_scores.keys()), list(conf_f1_scores.values()), Path(args.output_dir), plot_name=f"f1_curve_{args.tp_distance}.png")
