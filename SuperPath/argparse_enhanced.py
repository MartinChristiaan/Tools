...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        df_new = pd.DataFrame([vars(args)])
        print(df_new)
        argsdir = Path(f"{home}/.argparse/")
        argsdir.mkdir(exist_ok=True)
        original_file = sys.argv[0].split(".py")[0].split("/")[-1]
        filename = original_file + ".csv"
        print(filename)
        script_cache_path = argsdir / filename
        if script_cache_path.exists():
            df_old = pd.read_csv(script_cache_path)
            df_new = pd.concat([df_old, df_new])
            df_new = df_new.drop_duplicates()
        print(df_new)
        df_new.to_csv(script_cache_path, index=False)
        return args
