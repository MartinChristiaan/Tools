...
from datatypes import readSources,Source
...
class MyApp(Flask):
	def __init__(self, *args, **kwargs):
		...
		self.root_sources = [x for x in self.sources if len(x.parent_sources) == 0 and len(x.child_sources) > 0]
		CORS(self)
	...
	def set_favorite(self):
		...
		if len(favorite == 0):
			favorite = Source("favorites","",datetime.now(),-1,1,[],"favorites",[])
	...
	def get_random_items(self):
		...
		self.chosen_elements = random.choices(self.root_sources,weights=[x.weight for x in self.root_sources],k=items_per_request)
		return jsonify([weighed_select_leaf(x) for x in self.chosen_elements])
	
	def get_items_like(self):
		data = request.data
		source = self.source_lookup[data['id']]
		self.chosen_elements = random.choices(source.parent_sources,weights=[x.weight for x in source.parent_sources],k=items_per_request)
		return jsonify([weighed_select_leaf(x) for x in self.chosen_elements])
