...
def find_leaf_directories_with_substring(path, substring):
	...
	if os.path.isdir(path):
		...
		for item in os.listdir(path):
			item_path = os.path.join(path, item)
			if os.path.isdir(item_path):
				leaf_directories.extend(find_leaf_directories_with_substring(item_path, substring))
...
try:
	...
	annotation_files = find_files_with_string(path,"annotation")
	...
	if len(track_files):
		...
		tracks = Detections.load(annotation_files[0])
	print(tracks)
	...
	print('press a char to continue')
	c=  click.getchar()
	sys.exit()
