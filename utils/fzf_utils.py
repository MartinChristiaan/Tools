...
class SFzfPrompt(FzfPrompt):
	def prompt(self, choices=[],multi=False,prompt_text=None,return_idx=False,extra_options = ""):
		fzf_options = ""
		if multi:
			fzf_options += "--multi "
...
prompter = SFzfPrompt()
def prompt(choices=[],multi=False,prompt_text=None,return_idx=False,extra_options="",cachename=None):
	...
	if len(choices) == 1:
		...
		if multi:
			return choices
		return choices[0]
	result = prompter.prompt(choices,multi,prompt_text,return_idx,extra_options=extra_options)
	for item in result:
