...
def update_csv_with_dataclass(dataclass, csv_path):
	...
	if existing_headers != expected_headers:
		missing_headers = [header for header in expected_headers if header not in existing_headers]
		extra_headers = [header for header in existing_headers if header not in expected_headers]
		prompt = SFzfPrompt()
		...
		for header in missing_headers:
			...
			if not  click.getchar() == 'y':
				df[header] = None
				continue
			fill_header =  prompt.prompt(extra_headers,False,f'select header for {header}')
