...
def load_dataset(args):
    if args.name and not args.dataset_dir and not args.data_path and not args.labels_path:
    ...
    try:
        ...
        print(f'Dataset with name {args.name} already exists. Loading existing dataset.')
        return load_dataset_by_name(args.name), True
...
def main(args):
    ...
    if not by_name:
        ...
        for sample in frames:
            ...
            for field_name, value in dataset.get_field_schema().items():
                if isinstance(value, EmbeddedDocumentField) and value.document_type is Detections and "segmentation" not in field_name:
        ...
        if args.predictions_paths:
            ...
            for i, pred_path in enumerate(args.predictions_paths):
                prediction_name = args.predictions_names[i] if len(args.predictions_names) > i else f"{Path(pred_path).parents[1].name}_{Path(pred_path).parent.stem}"
            
                if Path(pred_path).exists():
                    print(f'Adding predictions from {pred_path} for model {prediction_name}...')
                    ...
                    if args.type == "coco":
                        ...
                        COCOVidDetectionDatasetImporter.add_coco_labels(dataset, predictions, prediction_name)
                else:
    ...
    if args.bbox_size is not None and args.bbox_size > 0:
        ...
        for field_name, value in dataset.get_field_schema().items():
            ...
            if isinstance(value, EmbeddedDocumentField) and value.document_type is Detections:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml_path",
                        help="Dataset yaml file path. Used for yolov5 loading")
    parser.add_argument("--data_path", help="Image data to visualise")
    parser.add_argument("--labels_path", help="Labels to visualise")
    parser.add_argument("--predictions_paths", nargs='*', help="Predictions to visualise")
    parser.add_argument("--predictions_names", nargs='*', default=[], help="Prediction names. if None use filenames")
    ...
    parser.add_argument("--bbox_size", type=int, default=None,
    ...
    parser.add_argument("--classes", nargs='*', default=None, help="Classes used by coco predictions. Used when predictions_paths is used")
    parser.add_argument("--file_names", type=str, nargs='*', default=None, help="file names to show")
    parser.add_argument("--split_size", type=int, default=-1, help="Split size for coco-vid")
