...
class ImageLoader:
	...
	def quarantine_images_in_directory(self):
		...
		self.image_paths = [x for x in self.image_paths if os.path.dirname(x) != image_directory]
	...
	def next_different_image(self):
		self.index = get_next_different_image_index(self.image_paths,self.index,previous=False)
	def previous_different_image(self):
		if self.index == 0:
			return
		self.index = get_next_different_image_index(self.image_paths,self.index,previous=True)
